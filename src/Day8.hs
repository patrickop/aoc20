...
data Opcode = Nop | Acc | Jmp deriving (Eq, Show)
...
tick state 
  | Just instruction <- (getInstruction state) = Just $ executeInstruction state instruction
  | otherwise = Nothing
...
b filename = do
  ...
  let allHistoriesFinite = map ( catMaybes . (takeWhileList (not . loopsOrTerminates) []) ) allHistories
